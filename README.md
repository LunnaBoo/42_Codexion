...
# Description
This project is a revamp of the old Philosophers project from 42, and very well known programming problem. Besides having its own particularities
...
Which is essential knowledge to any developer these days and C is the perfect language to teach concepts like this to us since it's very low level.
...
1. Number of coders is self explanatory. It represents the number of coders in the simulation and the number of dongles too (dongles are like the forks in original Philosophers).
2. Time to burnout means the time each coder can spend without compiling before burning out and ending the simulation.
3. Time to compile means the time each coder spends to compile.
4. Time to debug means the time each coder spends to debug.
5. Time to refactor means the time each coder spends to refactor.
6. Number of compiles required means how many times each coder has to compile to finish the simulation without burnouts.
7. Dongle cooldown is about the time a dongle stays inaccessible after it was used.
8. Scheduler refers to the arbitration policy used by dongles to decide who gets them when multiple coders request them. FIFO means First in First Out,
...
2. Hold-and-wait: Once a resource is obtained, a process keeps the resource locked while waiting for another.
...
Based on this, my solution aimed to fix the circular wait by ordering dongle requests in ascending dongle-id order. So instead of Coder 4 holding dongle 3 and waiting for dongle 0, it requests
...
The solution to race conditions. It is like a lock that protects a block of code and can only be executed by its owner until they unlock it themselves.
...
##### pthread_cond_t
It's a condition variable, the pthread type for "sleep until signaled" bell. Besides my description of it, a condition variable has no condition inside, it is just meant to send a signal. The condition itself is just data protected by a mutex. \
This has to be paired to a mutex to avoid race conditions (two or more threads changing the shared data) and avoid lost wake-up. The latter happens when a thread checks the condition == false, then the condition is changed (no mutex preventing it) and the thread sleeps anyway since it checked the condition and found it false.
